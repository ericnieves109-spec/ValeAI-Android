...
import { processIndividualFile, analyzeContentWithGemini, saveProcessedFile, checkInternetConnection } from "./fileProcessor.js";
const router = Router();
...
router.post("/api/generate-image", async (req, res) => {
    try {
        ...
        const enhancedPromptResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${InyeccionGemini25.config.model}:generateContent?key=${InyeccionGemini25.config.key}`, {
            method: "POST",
...
function generateEducationalSVG(topic, relatedTopic) {
...
Responde de forma clara, educativa y con tu personalidad característica en español.`;
            ...
            const relevantKnowledge = knowledge.filter(k => searchTerms.some(term => k.materia.toLowerCase().includes(term) ||
...
router.post("/api/process-file", upload.single("file"), async (req, res) => {
