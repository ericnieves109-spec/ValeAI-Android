...
export async function processIndividualFile(buffer: Buffer, filename: string): Promise<string> {
...
export async function analyzeContentWithGemini(content: string): Promise<{
  topics: string[];
  categories: string[];
  summary: string;
}> {
...
export async function saveProcessedFile(file: any, extractedContent: string, analysis: any) {
  const processedFile = {
  ...
  if (analysis.summary && analysis.categories.length > 0 && analysis.topics.length > 0) {
    const knowledgeEntries = analysis.topics.slice(0, 5).map((topic: string) => ({
      id: crypto.randomUUID(),
